 ...
 */
function cipp_breadcrumb($variables) {
 ...
 * Implements template_preprocess_zone().
 */
function cipp_process_zone(&$vars) {
  if ($vars['zone'] == 'content') {
    $node = menu_get_object();
    $theme = alpha_get_theme();
    $tabs = cipp_theme_process_tabs($theme->page['tabs']);
 ...
 */
function cipp_theme($existing, $type, $theme, $path) {
  return array(
    'cipp_tabs_local_task' => array(
 ...
 */
function cipp_theme_process_tabs($tabs) {
  if ($tabs['#primary']) {
    // Remove active tab.
    foreach ($tabs['#primary'] as $row_num => $items) {
      $tabs['#primary'][$row_num]['#theme'] = 'cipp_tabs_local_task';
  ...
  if ($tabs['#secondary']) {
    ...
    foreach ($tabs['#secondary'] as $row_num => $items) {
      $tabs['#secondary'][$row_num]['#theme'] = 'cipp_tabs_local_task';
    }
  }
  return $tabs;
}
/**
 * Implements theme_cipp_local_task().
 */
function cipp_cipp_tabs_local_task($variables) {
  ...
  elseif ($link['page_callback'] == 'cipp_dataset_add_resource') {
    $icon_type = 'plus';
  }
  elseif ($link['page_callback'] == 'cipp_dataset_back') {
    $icon_type = 'caret-left';
  }
  elseif ($link['page_callback'] == 'cipp_datastore_download') {
    $icon_type = 'download';
    $link['localized_options']['attributes']['class'][] = 'btn-primary';
  }
  elseif ($link['page_callback'] == 'cipp_datastore_datastore_api') {
    $icon_type = 'beaker';
 ...
 */
function cipp_horizontal_tabs($variables) {
 ...
 */
function cipp_facetapi_link_active($variables) {
 ...
 */
function cipp_facetapi_link_inactive($variables) {
 ...
 */
function cipp_preprocess_user_profile(&$variables) {
